  ...
  return (
    <ScreenContainer containerClassName="bg-[#F8F7FC]" safeAreaClassName="bg-[#F8F7FC]">
      <ContextualHint
        screen="budgets"
        locale={locale}
        titleKey="budgetsHintTitle"
        bodyKey="budgetsHintBody"
        icon="speed"
      />
      <ScrollView
      ...
      >
        ...
        >
          ...
          >
            <AppIcon name="speed" color="#7C5CFC" size={25} />
          </View>
        </View>

        {/* Total Budget Card */}
        <View style={{ backgroundColor: "#292044", borderRadius: 25, padding: 20, marginTop: 20, gap: 17 }}>
          <View
            style={{
              flexDirection: isRTL ? "row-reverse" : "row",
              alignItems: "center",
          ...
          >
            <View style={{ gap: 5, alignItems: isRTL ? "flex-end" : "flex-start" }}>
              <Text style={{ color: "#BEB6DC", fontSize: 11, fontWeight: "700" }}>
                {t(locale, "budgetControl")}
              </Text>
          ...
          <View style={{ height: 11, backgroundColor: "#443461", borderRadius: 10, overflow: "hidden" }}>
            <View
              style={{
          ...
          <View style={{ flexDirection: isRTL ? "row-reverse" : "row", justifyContent: "space-between" }}>
            <Text style={{ color: "#BEB6DC", fontSize: 11 }}>{t(locale, "budgetUsed")}</Text>
            <Text style={{ color: "#FFFFFF", fontSize: 11, fontWeight: "900" }}>
              {formatMoney(totalLimit, locale)} · {t(locale, "totalBudget")}
            </Text>
          </View>
        </View>
        ...
        >
          ...
          <Text style={{ color: "#9996A8", fontSize: 11, fontWeight: "700" }}>
            {budgets.length} دسته
          </Text>
        </View>
        ...
        >
          {budgets.map((budget, index) => (
            ...
            >
              ...
              >
                <AppIcon name="edit" color="#AAA6B8" size={16} />
              </Pressable>
            </View>
          ))}
        </View>
        ...
        >
          <AppIcon name="lightbulb-outline" color="#7C5CFC" size={20} />
          <Text
            style={{
              color: "#6856A9",
              fontSize: 11,
          ...
          >
            {overallPercent >= 80 ? t(locale, "budgetExceeded") : t(locale, "budgetRemaining")}
          </Text>
        </View>
      </ScrollView>
      ...
      >
        <View
          style={{
            ...
            justifyContent: "center",
            padding: 22,
            backgroundColor: "rgba(26,20,46,0.35)",
          }}
        >
          <View style={{ backgroundColor: "#FFFFFF", borderRadius: 24, padding: 20, gap: 16 }}>
            <View
              style={{
                flexDirection: isRTL ? "row-reverse" : "row",
                justifyContent: "space-between",
            ...
            >
              <Text style={{ color: "#28243E", fontSize: 18, fontWeight: "900" }}>
                {t(locale, "editBudget")}
              </Text>
              <Pressable onPress={() => setEditingId(null)}>
                <AppIcon name="close" color="#9996A8" size={22} />
              </Pressable>
            </View>
            <Text style={{ color: "#777487", fontSize: 12, textAlign: isRTL ? "right" : "left" }}>
              {t(locale, "budgetLimit")}
            </Text>
            <TextInput
              value={draftLimit}
            ...
            <Pressable
              onPress={save}
              style={({ pressed }) => [
                { backgroundColor: "#7C5CFC", borderRadius: 14, paddingVertical: 14, alignItems: "center" },
                pressed && { opacity: 0.8 },
              ]}
            >
              <Text style={{ color: "#FFFFFF", fontWeight: "900" }}>{t(locale, "saveBudget")}</Text>
            </Pressable>
          </View>
        </View>
      </Modal>
